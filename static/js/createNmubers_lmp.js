...
function pk10_groupList(showId) {
	...
	switch(showId) {
		case 'lm':
			...
			ids.forEach(function(item1, index1) {
				if(item1 == "gyds") {
					numbers[index1] = pk10_btnLabels[item1];
				} else {
					var keyNames = item1.split("_");
					var newsItems = []
		...
		case '1T5':
			...
			ids.forEach(function(item1, index1) {
				var keyNames = item1.split("_");
				var newsItems = []
				pk10_btnLabels[keyNames[1]].forEach(function(item2, index2) {
					var obj = {};
					obj.name = item2.name
					obj.selected = item2.selected
					obj.key = item2.key
		...
		case 'gyzh':
			...
			ids = ["gyzh"];
			ids.forEach(function(item1, index1) {
				var ary = pk10_btnLabels['gyds'].concat(pk10_btnLabels[item1])
				var newsItems = []
				ary.forEach(function(item2, index2) {
					var obj = {};
					obj.name = item2.name
		...
		case 'lm':
			...
			ids.forEach(function(item1, index1) {
				if(item1 == "zh_lhh") {
					...
					var keyNames = item1.split("_");
					var newsItems = []
						ssc_btnLabels[keyNames[1]].forEach(function(item2, index2) {
							var obj = {};
							obj.name = item2.name
							obj.selected = item2.selected
							obj.key = item2.key
...
function getNumbers(names, numbers) {
	var ary = [];
	numbers.forEach(function(item1, index){
