...
<script>
	...
	export default {
		...
		methods: {
			...
			...mapMutations("bet", ["saveStandGameList", "setGamePlayList", "saveLotteryData"]),
			...
			websoket() {
				...
				//				var websocket = new WebSocket(encodeURI(str + URL_1 + "/notify/" + this.userDetails.account));
			...
			chat_onMessage(message) {
				...
				var messageType = JSON.parse(message.data).messageType;
			...
			$route(to, from) {
				...
				if(routes.includes(from.path) && routes.includes(to.path)) {
					...
					if(this.routerHistory.includes(to.path)) {
						var index = this.routerHistory.findIndex((item, index) => {
						...
						var _index = this.routerHistory.findIndex((item, index) => {
