		...
		<div class="clockView">
			<div class="left" style="position: relative;">
				...
				<p style="display: flex;align-items: center;text-align: center;justify-content: center;">
					<img src="../../../../static/img/lingdang.png" style="width: 0.3rem;margin-right:0.1rem;" />第{{prevAwarPeriod2}}期</p>
				<ul>
			...
			<div class="right">
				...
				<CountDown class="clock-count" :format="':'" :classId="classId" :kindId="kindId" :historyData="historyData" v-if="historyData" style="font-size: 14px;color:#e43a45;" @stop="countDown"></CountDown>
			</div>
		</div>
				...
				<div :style="listInfo_style" class="listInfo" ref="wrapper3">
					<div class="tab-title">
						...
						<div class="right">
							...
							<a href="javascript:;" @touchend="tableType=4" :class="tableType==4?'active':''">质合</a>
						</div>
					</div>
						...
						<div class="tab-item" v-for="(item,index) in winList" v-if="item.awardNum!=null">
							...
							<div class="right">
								<div class="nums-wrap" v-if="tableType==1">
									...
									<span style="font-size: 14px;color: red;margin: 0.1rem width:0.45rem;text-align:center;">{{item.numSum}}</span>
									<span style="font-size: 14px;margin: 0.1rem;"><span>跨度：</span><span>{{item.spanNum}}</span></span>
								</div>
								...
								<div class="nums-wrap" v-if="tableType==4">
									<span :class="primeNumber.indexOf(item.awardNum.split(',')[0])==-1?'num-round orange':'num-round blue'">{{primeNumber.indexOf(item.awardNum.split(',')[0])==-1?'合':'质'}}</span>
									<span :class="primeNumber.indexOf(item.awardNum.split(',')[1])==-1?'num-round orange':'num-round blue'">{{primeNumber.indexOf(item.awardNum.split(',')[1])==-1?'合':'质'}}</span>
									<span :class="primeNumber.indexOf(item.awardNum.split(',')[2])==-1?'num-round orange':'num-round blue'">{{primeNumber.indexOf(item.awardNum.split(',')[2])==-1?'合':'质'}}</span>
								</div>
							</div>
						</div>
					</div>
...
<script>
	...
	export default {
		...
		computed: {
			...mapState("bet", ["subGameId", "leftName", "rightName", "gameId", "gamePlayList", "subGameList", "kindList"]),
		...
		methods: {
			...
			sumOfballs() {
				...
				if(this.awardNum) {
					resultArray = this.awardNum.slice(0)
					// resultArray = this.sort(resultArray)
					...
					this.ballSpan = lastNumber - firstNumber;
					console.log(lastNumber, firstNumber, '00000')
			...
			getSumOfballList(winList) {
				...
				winList.forEach((item) => {
					...
					if(item.awardNum) {
						...
						item.spanNum = lastNumber - firstNumber;
