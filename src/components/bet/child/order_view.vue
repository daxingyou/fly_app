<template>
	<transition name="fade2">
		<div class="order" v-if="option.ticketCount>0">
			<div v-if="selectModel==2" style="display: flex;justify-content: center;flex-direction: column;">
				<p style="text-align: center;">
					</span>共选择{{option.ticketCount}}注</p>
				<p>投注{{Math.round(option.totalPrice*1000)/1000}}元</p>
			</div>
			<div v-if="odd!=''&&odd!=null&&selectModel==1" style="display: flex;justify-content: center;flex-direction: column;">
				<p style="text-align: center;">当前赔率为{{odd}}，共选择{{option.ticketCount}}注，投注{{Math.round(option.totalPrice*1000)/1000}}元。</p>
				<p>最多中奖{{winAmount}}元，盈利{{profit}}元。</p>
			</div>
			<div v-if="odd==''&&odd!=null&&selectModel==1" style="display: flex;justify-content: center;flex-direction: column;">
				<p style="text-align: center;">
					</span>共选择{{option.ticketCount}}注，可中奖{{winAmount}}元</p>
				<p>投注{{Math.round(option.totalPrice*1000)/1000}}元，最多盈利{{profit}}元。</p>
			</div>
			<div v-if="odd==null&&selectModel==1" style="display: flex;justify-content: center;flex-direction: column;">
				<p style="text-align: center;">
					</span>共选择{{option.ticketCount}}注</p>
				<p>投注{{Math.round(option.totalPrice*1000)/1000}}元</p>
			</div>
...
<script>
	...
	export default {
		...
		computed: {
			...
			totalPrice: function() {
				return this.unitPrice * this.inputValue
			}
		},
		methods: {
			combination(m, n) {
			...
			getProfit_zuhe(newNumbers) {
				...
				var winAmount = 0;
				var profit = 0;
				var odd = this.subGameList[0].oddMax.split(",");
				// console.log(odd)
				var odd1 = odd[0] * 1;
				var odd2 = odd[1] * 1;
				var odd3 = odd[2] * 1;
				var winAmount_min = odd3 * this.inputValue * this.unitPrice;
				var winAmount_max = odd1 * this.inputValue * this.unitPrice + odd3 * this.inputValue * this.unitPrice + odd2 * this.inputValue * this.unitPrice;
				// console.log(winAmount_max,winAmount_min)
				var profit_min = winAmount_min - (this.inputValue * this.unitPrice * this.option.ticketCount);
				var profit_max = winAmount_max - (this.inputValue * this.unitPrice * this.option.ticketCount);
				// newNumbers.forEach((item, index) => {
				// 	if(index == 0) {
				// 		if(item.length > 1) {
			...
			getProfit_optional_dt(newNumbers, num) {
				...
				//						var count = this.combination(newNumbers[1].length,num-newNumbers[0].length)
