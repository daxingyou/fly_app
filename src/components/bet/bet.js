...
import { createSpecialBetNumbers, createBetNumbers, calcChooseBetCount } from './renderPalette.js';
...
var bjscSpecialArr = [];
//快三和值subGameId数组;
var ksSumValArr = ["400101", "400102", "400103", "400104", "400105", "400106", "400107", "400108", "400109", "400110", "400111", "400112", "400113", "400114", "400115", "400116", "400117", "400118", "400119", "400120", "400121", "400122", "400123", "400124"];
//快三1-3球subGameId数组;
var ksSpecialArr = ["401001", "401002", "401101", "401102", "401103"]
//胆拖subGameId数组
...
//buttom选号方式
export function onSelectBtn(obj, subGameId, data, bileCodeArr, optionalFields) {
	var noteNumber, newNumbers;
	var result;
	var row = obj.row,
		col = obj.col;
	var boolen = data.numbers[row]["cols"][col].data_selected
	//胆拖选号方式
	if(dantuoArr.indexOf(subGameId) != -1) {
		//胆码选择上限
		var dantuoMax = data.dantuoMax
		//点击之前的状态
		...
		if(!boolen) {
			if(row == "0") {
				...
				var length = bileCodeArr.length
				//判断bileCodeArr是否超过胆码选择上限
				if(length >= dantuoMax) {
					//要变为未选中的按钮的下标
					var oldIdx = bileCodeArr[0];
					//要变为选中的按钮的下标
					var newIdx = bileCodeArr[length - 1];
					//改变状态
				...
				if(bileCodeArr.indexOf(col) != -1) {
					var idx = bileCodeArr.indexOf(col)
		...
		if(boolen == true) {
			...
			for(var i = 0; i < data.numbers.length; i++) {
				for(var k = 0; k < data.numbers[i].cols.length; k++) {
		...
		var inverse = data.numbers[row]["cols"][col].inverse;
		switch(inverse) {
			//大小单双质合互斥
			case 0:
				if(boolen) {
...
function mutex(obj, num, boolen, data) {
	var row = obj.row,
		col = obj.col;
	for(var i = 0; i < data.numbers[row]["cols"].length; i++) {
		if(data.numbers[row]["cols"][i].inverse == num) {
			data.numbers[row]["cols"][i].data_selected = !boolen
		}
	}
}
export function getLotteryData_lmp(data) {
	var LotteryData = [];
	data.newNumbers.forEach((item, index) => {
		var obj = {};
		obj.oddMax = data.twoGameOddList[item.key];
		obj.periodNo = data.periodNo;
		obj.price = data.price;
		obj.key = item.key.split('_')[0];
		obj.selecteNum = item.key.split('_')[1];
		obj.subGameId = data.subGameId;
		obj.subGames = item.title;
		obj.ticketCount = 1;
		obj.totalPrice = data.price * data.ticketCount;
		obj.unite = data.unite;
		LotteryData.push(obj)
	})
	return LotteryData
}
//处理购彩蓝数据
export function getLotteryData(data, checkAllGroup, selectMode) {

	var specialArys1 = bjscSpecialArr.concat(ksSumValArr);
	var specialArys2 = bjscSpecialArr.concat(ksSumValArr, SscSpecialArr, specialNum);
	var {
		unite,
		price,
		totalPrice,
	...
	if(specialArys2.indexOf(subGameId) != -1) {
		...
		var LotteryData = new Object();
		//单式
		var str = '';
		var o = checkAllGroup;
		if(checkAllGroup.length != 0) {
			var compareData = {
				"万位": 1,
				"千位": 2,
				"百位": 3,
			...
			checkAllGroup = o.sort(function(a, b) {
		...
		if(["200301", "200302", "200303"].indexOf(subGameId) != -1 || hk6BetNumbersConfig.banbo[subGameId]) {
			...
			LotteryData.push(obj)
			return LotteryData
		} else if(subGameId == '401101'||subGameId == '401001') {
			var LotteryData = [];
...
//			obj.ticketCount = 0;
			newNumbers.forEach((item, index) => {
				...
				if(item.length != 0) {
					item.forEach((it,idx)=>{
						it = betDataList.numbers[index]["cols"][it]["label"];
						obj.selecteNum += it+',';
						obj.ticketCount ++;
					})
					obj.selecteNum = obj.selecteNum.slice(0,obj.selecteNum.length-1)
					if(subGameId == '401101'){
						if(index == 0){
						obj.subGames = leftName+'(第一球)';
					}else if(index == 1){
						obj.subGames = leftName+'(第二球)';
					}else if(index == 2){
						obj.subGames = leftName+'(第三球)';
					}
					}else{
						if(index == 0){
						obj.subGames = leftName+'(必出)';
					}else if(index == 1){
						obj.subGames = leftName+'(不出)';
					}
					}
					obj.oddMax = subGameList[index].oddMax;
					obj.subGameId = subGameList[index].subGameId;
					obj.totalPrice = Math.round(price * 1 * obj.ticketCount * 100) / 100
					obj.price = price;
					obj.unite = unite;
					obj.periodNo = periodNo;
					obj.chaseNum = 1;
			...
			if(selectMode == "0") {
				...
				var str = ary.join('') + '|';
				LotteryData.selecteNum = str + newNumbers.join(",");
			}
...
export function chaseBets(lotteryData, cachingArray) {
	var arr = [];
...
export function quickBets(data) {
	var params = {};
	var {
		lotteryData,
		kindId,
	...
	lotteryData.forEach((item) => {
		...
		obj.unitPrice = Math.round(item.price * item.unite * 1000) / 1000;
		obj.totalAmount = Math.round(obj.unitPrice * obj.orderNum * 1000) / 1000;
	...
	params.cost = Math.round(cost * 1000) / 1000;
	return params
}
export async function renderingData(str, standGameList, gameId) {
	var oddTxtShow = true;
	var leftName = str.split('-')[0],
		rightName = str.split('-')[1],
		subGameId = str.split('-')[2];
	var data, subGameList;
	var subGameIds, gameSons;
	var params = {};
	var betTypeMap;
	var indexAry = [];
	if(SscSpecialArr.indexOf(subGameId) != -1) {
		...
		//		var res = await request('post', 'getSubGames', params)
		//		subGameList = [];
	...
	} else if(ksSpecialArr.indexOf(subGameId) != -1) {
		if(subGameId != "401001" && subGameId != "401002") {
			subGameIds = ksSpecialArr.slice(2)
		} else {
			subGameIds = ksSpecialArr.slice(0, 2)
		}
		oddTxtShow = false;
		//		params.gameIds = subGameIds;
		//		var res = await request('post', 'getSubGames', params)
		...
		switch(leftName) {
			...
			case "冠亚合值":
				...
				break
			case "冠亚组合合值":
				subGameIds = bjscSpecialArr.slice(4)
...
//快捷选号
export function quickBtnSelect(data, obj, optionalFields) {

	var betDataList = data,
		value = obj.value,
		row = obj.row;

	var middleNumber = betDataList.max / 2;
	switch(value) {
		case 'all': //全选
			for(var k = 0; k < betDataList.numbers[row].cols.length; k++) {
