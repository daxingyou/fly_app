<template>
	<div class="chase" v-if="show" :style="{width:'100%',height:bodyHeight+'px'}">
		<yd-popup v-model="show3" position="center" width="90%" :close-on-masker="false">
			<div class="set_chase">
				...
				<div class="conent">
					<div class="item1">
						<p>连续追号
							<yd-spinner :max="maxChase" unit="1" width="1.4rem" height=".5rem" v-model="chasePeriods" :longpress="false"></yd-spinner>期</p>
						<p>起始倍数
							<yd-spinner max="10000" unit="1" width="1.4rem" height=".5rem" v-model="chaseTimes" :longpress="false"></yd-spinner>倍</p>
					</div>
					...
					<yd-radio-group class="radio_sel" v-model="chaseWinType" color="#d91d36">
						<yd-radio val="1">全程最低盈利率
							<yd-spinner max="10000" unit="1" width="1.4rem" height=".5rem" v-model="chaseMinPercent" :longpress="false"></yd-spinner>%</yd-radio>
						<yd-radio val="3">全程最低盈利
							<yd-spinner max="10000" unit="1" width="1.4rem" height=".5rem" v-model="chaseMinNumber" :longpress="false"></yd-spinner>元</yd-radio>
						<yd-radio val="2">前
							<yd-spinner max="10000" unit="1" width="1.4rem" height=".5rem" v-model="chaseBeforePeriod" :longpress="false"></yd-spinner>期
							<yd-spinner max="10000" unit="1" width="1.4rem" height=".5rem" v-model="chaseBeforePercent" :longpress="false"></yd-spinner>% &nbsp;之后盈利率
							<yd-spinner max="10000" unit="1" width="1.4rem" height=".5rem" v-model="chaseAfterPercent" :longpress="false"></yd-spinner>
						</yd-radio>
					</yd-radio-group>
		...
		<yd-navbar>
			<router-link :to="'/buyColorBasket/'+classId+'/'+this.$route.query.kindId" slot="left">
		...
		<div class="setup" v-show="confirmBetType=='2'">
			<div class="left">
				追号
				<yd-spinner :longpress="false" :max="maxChase" unit="1" v-model="chasePeriods"></yd-spinner>期
			</div>
			<div class="right">
				起始倍数
				<yd-spinner :longpress="false" max="10000" unit="1" v-model="chaseTimes"></yd-spinner>
			</div>
		</div>
		<div class="setup_gj" v-show="confirmBetType=='3'">
			<div class="left">
				<p>追号
					<yd-spinner max="10000" unit="1" v-model="chasePeriods" :longpress="false"></yd-spinner>期</p>
					...
					<tr v-for="(item,index) in chaseList">
						...
						<td>
							...
							<div style="display: flex; justify-content: center; align-items: center;">投
								<span class="yd-spinner" style="height: 0.5rem; width: 1.4rem;"><input type="number"  pattern="[0-9]*" placeholder="" class="yd-spinner-input" @input="changeTimes(index)" @blur="chaseList[index].times==''?chaseList[index].times=1:chaseList[index].times" v-model="chaseList[index].times" /></span>倍</div>
						</td>
						<td>{{item.hasPay}}</td>
						<td>{{item.profit}}</td>
...
<script>
	...
	export default {
		...
		created() {
			// console.log(this.kindId,'1')
			this.classId = this.$route.query.classId;
			// this.kindId = this.routerID[this.$route.query.kindId] ;
			// console.log(this.kindId,'2')
			this.kindId = this.routerID[this.$route.query.kindId];
			console.log(this.kindId, '3')
			if(this.lotteryData[this.selectModel].length == 0) {
				this.$router.push({
					path: "/bet/" + this.classId + '/' + this.$route.query.kindId,
				})
			} else {
				this.show = true;
				...
				// console.log(this.new_lotteryData)
				this.orderNum = this.$route.query.data.selecteNum
				this.chasePeriods = this.$route.query.data.chaseNum;
				// this.chaseTimes = this.$route.query.data.price*1;
				// console.log(this.chaseTimes)
				this.goChase(true)
			}
		},
		computed: {
		...
		beforeRouteEnter(to, from, next) {
			if(from.name !== 'buyColorBasket') {
				next(vm => {
					vm.$router.push('/buyColorBasket/'+to.query.classId+'/'+to.query.kindId)
				})
			}else{
				next()
			}
		...
		methods: {
			...
			bet() {
				...
				// this.kindId = this.lotteryId.indexOf(this.kindId)
				if(this.noClick) {
					return false;
				}
				this.noClick = true
				...
				this.quickBet(params).then((res) => {
					if(res.code == "0") {
						...
						this.getUserDetails()
						this.$router.push({
							path: '/noterecord?type=2'
						})
						//						this.$router.push({
						//					path:"/buyColorBasket/"+this.classId+'/'+this.$route.query.kindId,
						//					
						//				})
			...
			changeTimes(index) {
				...
				if(!reg.test(str)) {
					...
					this.chaseList[index].times = 10000
				}
				if(str == '') {
					this.chaseList[index].times = '';
				}
				this.goChase(false)
			},
			changeType(type) {
			...
			async goChase(isFirst1) {
				...
				obj.totalPrice = this.new_lotteryData.unite * this.new_lotteryData.ticketCount * this.new_lotteryData.price;
				obj.noteNumber = this.new_lotteryData.ticketCount;
				obj.kindId = this.kindId;
				Object.assign(obj, this.subGameList[0])
				betList.push(obj)
				//				})
				//    console.log(confirmBetFields,chaseList,gameType,betList)
