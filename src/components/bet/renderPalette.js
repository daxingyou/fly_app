...
export function createSpecialBetNumbers(props) {
	...
	subGameIds.forEach(function(subGameId) {
		...
		}else if(KSBetNumbersConfig.integration[subGameId]){
			labels = ["1","2","3","4","5","6"]
		}
...
export function calcChooseBetCount(betNumbers, optionalFields) {
	...
	if(calcBetCountConfig.hk6Special[subGameId] || calcBetCountConfig.hk6hexiao[subGameId]) {
		switch(subGameId) {
			...
			case '201010':
			case '201011':
				var numbersLg = subGameId.slice(4, 6) * 1;
...
function loopNumbers_bet(_a) {
	...
	labels.forEach(function(item) {
		...
		if(blueLabels.indexOf(item) != -1 || ["03", "04", "09"].indexOf(item) != -1) {
			numbers.push({
				label: item,
				data_selected: false,
				notCircle: notCircle,
		...
		} else if(redLabels.indexOf(item) != -1 || ["01", "02", "07", "08"].indexOf(item) != -1) {
		...
		} else if(greenLabels.indexOf(item) != -1 || ["05", "06"].indexOf(item) != -1) {
			numbers.push({
				label: item,
				data_selected: false,
				notCircle: notCircle,
				...
				case "200901": // 一肖 一肖
					{
						...
						for(var i = 0; i < 12; i++) {
							if(i == animal) {
								var first_number = 1;
								var item = [String(first_number), String(first_number + 12), String(first_number + 24), String(first_number + 36), String(first_number + 48)];
								labels.fill(item, i, i + 1);
							} else {
								var first_number = i < animal ? (animal - i) + 1 : 13 - (i - animal);
								var item = [String(first_number), String(first_number + 12), String(first_number + 24), String(first_number + 36)];
