...
export function createChaseList(_a) {
	...
	if(confirmBetType === confirmBetTypeEnum.common) {
		...
		newChaseList.forEach(function(item) {
			...
			currentPay = parseFloat(currentPay.toFixed(3));
			hasPay_1 += currentPay;
			hasPay_1 = parseFloat(hasPay_1.toFixed(3));
			var profit = {
				profit: '--',
				profitRate: '--'
			};
		...
		if(chaseWinType === 1) {
			newChaseList.forEach(function(item, index) {
				...
				if(times !== 0 && times < 99999) {
					...
					currentPay = parseFloat(currentPay.toFixed(3));
					hasPay_2 += currentPay;
					hasPay_2 = parseFloat(hasPay_2.toFixed(3));
...
function calcProfitAndRate(_a) {
	...
	return {
		profit: real,
		profitRate: fhRound(real / hasPay * 1000)
	}
};
...
function calcChaseTimesByProfitRate(_a) {
	...
	var oddMin = oddArray[0];
	var betTimes = calcBetTimes(betItem);
	var profitRateMax = (oddMin * betTimes - totalAmount) / totalAmount * 1000;
	if(hasPay === 0) {
	...
	var chaseTimes = Math.ceil(hasPay * (profitRate / 1000 + 1) / (oddMin * betTimes - totalAmount - profitRate * totalAmount / 1000));
