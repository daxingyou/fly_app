<template>
	<div class="kindAside">
		...
		<div tabindex="1" :class="show?'lottery_type_name show':'lottery_type_name'" v-show="show">
			...
			<yd-accordion>
				<yd-accordion-item :title= item.className  v-for="(item,indexI) in list" :key="indexI" :open="indexI==6?true:false">
				...
				<div style="padding: .24rem;">
					...
					class="kindName" 
					v-for="(childList,indexII) in list[indexI].kindList" :key="indexII">{{childList.kindName}}</p>
				</div>
				</yd-accordion-item>
			</yd-accordion>
...
<script>
	...
	export default {
		...
		created(){
			...
			this.changeKindId( this.$route.params.kindId.split("-")[1],'75秒赛车',6)
		},
		computed:{
			...mapState('twobet', [
				'twoGameList'
		...
		methods: {
			...
			changeKindId(to,name,id){
				var toPush ='/twobet/'+this.class_id[id]+'-'+to;
