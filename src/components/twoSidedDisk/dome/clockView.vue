			...
			<div class="lr_div ">
				<div class="lr_number">
					...
					<div class="lrn_number num-HK6" v-if="classId == 'HK6'">
						<div class="div_number">
							<div class="ball-green resultitem number-38">{{awardNumber[0][0]}}</div>
							<div class="ball_fonts">{{sx[0]}}</div>
						</div>
						<div class="div_number"><div class="ball-blue resultitem number-10">{{awardNumber[0][1]}}</div><div class="ball_fonts">{{sx[1]}}</div></div>
						<div class="div_number"><div class="ball-green resultitem number-11">{{awardNumber[0][2]}}</div><div class="ball_fonts">{{sx[2]}}</div></div>
						<div class="div_number"><div class="ball-blue resultitem number-4">{{awardNumber[0][3]}}</div><div class="ball_fonts">{{sx[3]}}</div></div>
						<div class="div_number"><div class="ball-blue resultitem number-3">{{awardNumber[0][4]}}</div><div class="ball_fonts">{{sx[4]}}</div></div>
						<div class="div_number"><div class="ball-blue resultitem number-14">{{awardNumber[0][5]}}</div><div class="ball_fonts">{{sx[5]}}</div></div>
						<div class="div_number"><div class="plus">+</div></div>
						<div class="div_number"><div class="ball-green resultitem number-28">{{awardNumber[0][6]}}</div><div class="ball_fonts">{{sx[6]}}</div></div>
					</div>
...
export default {
	data(){
		return{
			...
			hk6:{1:'狗',12:'猪',11:'鼠',10:'牛',9:'虎',8:'兔',7:'龙',6:'蛇',5:'马',4:'羊',3:'猴',2:'鸡'},
			year:null,
			sx:[],
		}
	},
	...
	methods:{
		...
		_getWinsDragon(classI,kind){
			...
			this.getWinsDragon('/'+kind).then( res => {
				...
				var num = res.data.winList[0].awardNum;
				if(classI == 'PK10'){
					if(num == null){
					...
					if(num == null){
						that.awardNumber.push(['02','24','08','26','42','30','45']);
					}else{
						that.awardNumber.push(num.split(','));
					}
				}
		...
		longhu(classI,awardNumber){
			...
			if(classI == 'PK10'){
				...
				this.sx = [];
				awardNumber[0].forEach( item => {
					var ind = (parseInt(item)%12);
					var  yy = this.year-2018;
					if(ind == 0){
						ind = 12;
					}
					if(ind<yy){
