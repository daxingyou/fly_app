...
<script>
	...
	export default {
		...
		watch: {
			...
			betNumber(){
				if(this.betNumber == ''){  // 判断等于空的时候
					this.allBetNumber = 0;  // 给总价赋值为0
					this.shopList.forEach(item => {  // 然后遍历数组给所有子输入框赋值为0
							item.amount = 0;	
					}); 
				}else{                       // 判断不等于空的时候
					if(this.shopList){        // 且有数组时
						this.shopList.forEach(item => {   // 对数组遍历
							item.amount = this.betNumber*1;   // 子输入框的值等于总输入框的值，*1是转换为number类型
							this.allBetNumber += parseInt(item.amount*1)   // 再次计算总价
						});
						return this.allBetNumber;   // 返回一个总价
					}else{					// 且没有数组时
						this.allBetNumber = 0;    // 直接复制总价为0
					}
				}	
			},
			shopList:{           // 监听数组
				handler(newValue){   // handler方法对改变的新数组（newValue）进行监听
					this.allBetNumber = 0;   // 初始给总价赋值为0；
					newValue.forEach(item => {  //遍历新数组
						if(item.amount == ''){  // 如果有等于空的情况就赋值为0，主要是string类型与number类型相加回等于NAN
