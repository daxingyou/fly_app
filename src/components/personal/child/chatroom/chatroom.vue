<template>
  <div id="chartroom">
    <yd-navbar :title="titles[$route.meta.order]">
      <a @click="$router.push('/home')"   slot="left">
    ...
    <Pop width="auto" v-model="show" bgcolor="#fff">
      <ul class="List" slot="content">
        <li>
          <Box @click.native="$router.push('/addFriend')" width="auto" height="auto" backgroundColor="#fff" colorEffect="rgba(0, 0, 0, 0.103)">
            <i class="fa fa-user-plus"></i> 加好友 / 群</Box>
          <Box @click.native="$router.push('/establishGroup')" width="auto" height="auto" backgroundColor="#fff" colorEffect="rgba(0, 0, 0, 0.103)">
            <i class="fa fa-comment"></i> 创建群聊</Box>
        </li>
      </ul>
    </Pop>
    ...
    <div class="chartroom_footer">
      ...
      <yd-tabbar active-class="tabactive" active-color="#d91d36">
         <yd-tabbar-item title="首页" type="link" link="/allKinds">
          <i slot="icon" class="fa fa-home" style="font-size:0.54rem"></i>
        ...
        <i class="fa fa-user-circle fa-lg" slot="icon" style="font-size:0.54rem;margin: 3px 0;"></i>
        </yd-tabbar-item>
      </yd-tabbar>
    </div>
  </div>
