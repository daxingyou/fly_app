        ...
        <yd-navbar title="群聊成员">
          <a @click="$router.go(-1)"  slot="left">
            <yd-navbar-back-icon ></yd-navbar-back-icon>
             ...
             <i  @click="show=true" name="more" slot="right" class="fa fa-navicon" style="font-size:.4rem;color:#fff;"></i>       
        </yd-navbar>
        <div class="container">
            <div class="group_owen">
                <p style="padding:.2rem;border-bottom: 1px solid #dcdcdc;">群主</p>
                <MessageBox  @click.native="queryData(groupLoader)" :name="groupLoader?groupLoader.nickName:'***'">
                <div id="_img" slot="img" :class="'sprite-icon-'+groupLoader.icon"  alt=""></div>
                <span v-show="groupLoader?groupLoader.memberAccount===userDetails.account:false" slot="badge" style="color:#4b4b4b">我</span>
            </MessageBox>
            </div>
            <div class="group_members">
                ...
                <div id="_img" slot="img" :class="'sprite-icon-'+item.icon"  alt=""></div>
        ...
        <yd-popup v-model="show" position="bottom" height="1rem">
           <Box v-if="authority" @click.native="$router.push({name:'inviteGroup',query:{groupId:$route.params.groupId}})" height="1rem" backgroundColor="#fff" colorEffect="#d3d3d3">
             <p>
               ...
               <i  name="ucenter-outline" class="fa fa-user-plus" style="font-size:.4rem;color:#525252;"></i>  
