    ...
    <div id="chartmessage">     
        <div class="container">
            <MessageBox @click.native="query(item.fromSource,item.singleOrGroup,'1',item.nickName)" v-for="(item,index) in noticeList" :key="index" :name="item.messageType==='4'?'通知':item.nickName" :time="global._timeFormate(item.sendTime,'1')" mode='1'>
              <div v-if="!item.accountProperties||item.accountProperties==='1'" id="_img" slot="img" :class="item.messageType==='1'&&item.singleOrGroup==='1'?'sprite-icon-'+item.icon:''" :style="{'background-image':item.messageType==='1'&&item.singleOrGroup==='2'?`url(../../../../../static/img/chartroom/${item.icon}.png)`:item.messageType==='4'?'url(../../../../../static/img/chartroom/notice.png)':''}" alt=""></div>
