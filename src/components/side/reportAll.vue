<template>
  <div id="reportAll">
    ...
    <yd-popup v-model="showDetail" position="center" width="90%" :close-on-masker="false">
      <div class="model">
        <h3>注单明细
          <!-- <yd-icon @click.native="showDetail=false" name="error-outline" size=".45rem"></yd-icon> -->
          <img @click="showDetail=false" src="../../../static/img/shanchu.png" alt="">
        </h3>
        <ul>
          ...
          <li><span style="float: left;width: 27%;">下注号码：</span>
            <span style="float: left;width: 73%;">
                <textarea readonly style="border: none;display:block;width: 100%;min-height: 1rem;line-height:12px;padding-top: 0.15rem;font-size: 12px;" v-model="dataDetail.lotteryNumbers"></textarea>
            </span>
          </li>
        </ul>
      </div>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      console.log(this.status, this.dayTime, this.account, this.kindType)
