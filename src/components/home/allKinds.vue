    ...
    <div class="top">
      <yd-navbar title>
        ...
        <div slot="right" v-else style="width:100%;height: 45px;padding-top: 6px;">
          <!-- <button class="top_but to_register" >app下载</button> -->
            ...
            <div class="lotteryTypes">
              <ul  @click="delegation">
                <li v-show="item[0]==='热门'||item[0]==='棋牌'" @click="clickQp">
                  ...
                  <section>
                    <p>极速牛牛</p>
                    <p style="color: #666666;font-size: 0.24rem;">全天{{gameConfig['klft'].periodNoAll}}期</p>
                    <p style="color: #666666;font-size: 0.23rem;">07:00-06:00</p>
                  </section>
                </li>
                <li v-for="(item,index) of item[1]"  :key='index' @click="result(item)" v-show="item.name != '重庆时时彩'">
                  <img :src="item.imgSrc" alt="">
                  <section>
                    <p>{{item.name}}</p>
                    <p style="color: #666666;font-size: 0.24rem;">全天{{gameConfig[item.baseLotteryId].periodNoAll}}期</p>
...
//      marginLefts:["hk6","fc3d","pl3","bjpk10"],
      ...
      // })
		...
		computed: {
			...
			...mapState(["isApp",'isLogin','queryAccout']),
		},
		methods: {
			...
			...mapActions("bet", ["getGamePlayList","getWinsDragon"]),
      ...
      if (list.length !== 0) {
        ...
        for (var item of set) {
          ...
          for (var item2 of list) {
            item2.imgSrc = 'static/img/icon/'+item2.baseLotteryId+'.png'
        ...
        return map;
			...
			result(obj) {
				this.setKindId(obj.baseLotteryId);
				Promise.all([
					this.getWinsDragon("/" + obj.baseLotteryId), //获取开奖历史
					this.getGamePlayList(`/${obj.baseLotteryClassId}/${obj.baseLotteryId}`), //获取小彩类玩法
				]).then((values) => {
					if(values[0].code === 0){
						//缓存开奖数据
					...
					if(obj.baseLotteryClassId == "2") {
					this.$router.push(`/hk6bet/${obj.baseLotteryClassId}/${this.routerID[obj.baseLotteryId]}`);
				} else {
					this.$router.push(`/bet/${obj.baseLotteryClassId}/${this.routerID[obj.baseLotteryId]}`);
				}
				})
