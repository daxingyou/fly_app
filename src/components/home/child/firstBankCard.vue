<template>
  <div class="firstBankCard">
    <yd-navbar title="绑定首张银行卡">
      <!-- <router-link to="/bankCard" slot="left">
        <yd-navbar-back-icon></yd-navbar-back-icon>
      </router-link> -->
      <yd-navbar-back-icon @click.native="$router.go(-1)" slot="left"></yd-navbar-back-icon>
    </yd-navbar>
    <div class="firstBankCard_nav">
      <yd-cell-group >
        <yd-cell-item>
          <span slot="left">开户姓名：</span>
          <yd-input slot="right" v-model="input_name" required   placeholder="请输入姓名"></yd-input>
        </yd-cell-item>
      </yd-cell-group>
      <yd-cell-group>
      ...
      <yd-cell-group v-if="show2 == false">
        <yd-cell-item>
          ...
          <yd-input slot="right" v-model="input_phone" required regex="/^1(3|4|5|7|8)\d{9}$/" placeholder="请输入电话号码"></yd-input>
        </yd-cell-item>
      </yd-cell-group>
    ...
    <div class="bankCard_ok" v-show="model3">
      <div>
        ...
        <button @click="toPash('/bankCard')">确认</button>
      </div>
    </div>
  </div>
</template>
...
<style>
  ...
  .firstBankCard .yd-navbar{
    background: linear-gradient(135deg,rgba(19,46,123,1) 0,rgba(0,201,202,1) 100%) !important;
  }
  .firstBankCard .yd-navbar .yd-navbar-item i{
    color: #ffffff !important;
  }
  .firstBankCard .yd-navbar span{
    color: #ffffff !important;
  }
  .firstBankCard_nav .yd-cell-box {
    margin-bottom: .15rem !important;
  }
  .firstBankCard_nav .yd-cell:after{
    border: 0 !important;
  }
  .firstBankCard_nav .yd-btn-block{
