  ...
  <div class="noteRecord">
    <div>
      ...
      <!-- <router-link  slot="left"> -->
        <yd-navbar-back-icon @click.native="goBack" slot="left"></yd-navbar-back-icon>
      <!-- </router-link> -->
    ...
    <ul class="noteRecord_head">
      <li :class="(changeIndex===1||changeIndex===4||changeIndex==5)?'active':''" @click="changeIdx(1,kindId)">注单查询</li>
      ...
      <!-- <li :class="changeIndex==5?'active':''" @click="changeIdx(5,kindId)">今日结算</li> -->
      <li >
    ...
    <div class="noteRecord_bav"  >
      <div v-show="nav_text" class="text" :class="noData?'no-data':''">
      	
        <div class="noteRecord_bav_but" v-if="this.changeIndex!= 3" v-show="nav_choic"  >
          <div class="qx" @click="_allbut()">
          	<input id="inp_all" class="inp_but " type="checkbox" />
          	<span>全选</span>
            <!--<yd-checkbox v-model="checkbox1">全选</yd-checkbox>-->
          </div>
          <div class="but" v-if="this.changeIndex === 1 || this.changeIndex === 4 ||this.changeIndex == 5">
            <button class="but_II" :class="changeIndex===1?'but_I':''" @click="changeIdx(1,kindId)">全部</button>
             <button class="but_II" :class="changeIndex===4?'but_I':''" @click="changeIdx(4,kindId)">未结</button>
              <button class="but_II" :class="changeIndex===5?'but_I':''"  @click="changeIdx(5,kindId)" >今日已结</button>
          </div>
          <div class="cd" @click="_cancelOrder(changeIndex,'')">
      ...
      <div style="background-color:#fff;">
        ...
        <ul class="popup_ul">
          ...
          <li>
            ...
            <span>下注期号</span>
            <span>:</span>
            <span>{{lotteryReportList.periodNo}}</span>
          </li>
          <li>
            <span>注数</span>
            <span>:</span>
          ...
          <li style="margin-bottom: 0.1rem;">
            <span>下注号码</span>
            <span>:</span>
            <textarea readonly style="border: none;display:block;width: 70%;height: 1rem;line-height:12px;margin-top:0.1rem;padding-top: 0.15rem;font-size: 12px;">{{lotteryReportList.lotteryNumbers}}</textarea>
          </li>
        </ul>
      </div>
...
<script>
  ...
  export default {
    ...
    created(){
    	//判断是否从购彩篮或追号页面进入
    	if(window.location.hash.split("?status=")[1] == '4'){
    		this.changeIndex = 4
    		this._queryLotteryOrderOwn('',1,'4');
    	}else if(window.location.hash.split("?type=")[1] == '2'){
    		this.changeIdx(2,this.kindId)
    	}else{
...
//                that.no_zh = false;
	    ...
	    _allbut(){
	    	 //获取checkbox按钮组
 					...
 					this.all = document.getElementById("inp_all");
 					 //全选按钮被选中时，遍历所有按钮
			    for(var i=0;i<this.allpro.length;i++){
	    ...
	    _cancelOrder(ind,id){
	    	var that = this;
        //筛选选中状态的投注
        var che = document.getElementsByClassName("inp_list");
      ...
      loadList() {
        ...
        this.queryLotteryOrderOwn({
          ...
          var _list = response.data.lotteryOrder;
...
<style >
  .noteRecord .yd-navbar-center-box {
  ...
  .text.no-data{
  	background:url(../../../../static/img/node_recore/zhh_no_img_03.png) no-repeat center center;
  	
  }
  .text.no-data .no_text{
   position: absolute;
   margin-top: 3rem;
   margin-left: 2rem;
  }
  .text.no-data .no_text .yd-btn{
  	width: 50%;
  }
  .noteRecord_head{
    overflow: hidden;
  ...
  .noteRecord_bav_but .but{
    float: left;
    width: 60%;
  }
  .noteRecord_bav_but .cd{
    float: right;
    width: 20%;
  }
  .noteRecord_bav_text{
